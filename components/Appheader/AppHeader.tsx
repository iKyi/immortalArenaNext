...
const AppHeader: React.VFC<AppHeaderPropsType> = ({ children, seo }) => {
    ...
    return (
        <>
            <Box component="header" sx={{ position: 'fixed', top: 0, left: 0, width: '100%', zIndex: 3 }}>
                <SeoComp seo={seo} />
                ...
                <Box sx={{
                    ...
                    {!Mobile ? navItems.map(item => {
                        return (
                            <HeaderNavButton key={item.url} active={router.pathname === item.url} onClick={() => router.push(item.url)}>
                                {item.name}
                            </HeaderNavButton>
                        )
                    }) : (
                        <Button
                            ...
                            variant="contained" onClick={() => setMobileOpen(true)}>
                            <MenuIcon />
                        </Button>
                    )}
                </Box>
